...
<%
	...
	int pageSize = 10; //페이지당 보여질 레코드 수 (개발자가 임의로 지정가능)
	int totalPage = (int)Math.ceil((float)totalRecord/pageSize); //총페이지 수
	int blockSize=10; //블럭당 보여질 페이지 수 (개발자가 임의로 지정가능)
	int currentPage=1; //현재페이지 (디폴트는 첫 페이지다! 따라서 1)
...
<body>
	<table>
		...
		<tr onMouseOver="showColor(this)" onMouseOut="hideColor(this)">
			<td><%=num-- %></td>
			<td>
			<%if(news.getWriter().length()<1){//삭제된 게시물이라면 글씨만 출력(문자열 길이가 0이라면) %>
