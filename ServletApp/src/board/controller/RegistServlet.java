...
public class RegistServlet extends HttpServlet{
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		//클라이언트의 요청에 multipart/form-data 방식이 포함될 경우 텍스트가 아닌 바이너리 파일로 전송된는
		//것이다. 따라서 업로드 컴포넌트를 사용해야한다
		...
		try {
			...
			Board board = new Board(); //empty VO준비
			boolean flag = false; //파일 업로드 성공 여부 
			
			for(FileItem item :items) {
				if(item.isFormField()) {//일반텍스트 컴포넌트라면 ,,, 작성자,제목, 내용들 textfield
					//vo에 알맞에 담자
