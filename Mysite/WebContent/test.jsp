...
/*
JNDI란?
		Java Namind Directory Interface : 
			어떤 정보를 프로그래밍 언어인 자바코드에 넣지말고, 외부의 xml과 같은 자원으로 관리하는 방법
			즉, 자바코드안에 설정정보를 넣지 말고, 코드 밖으로 빼서 유지관리하자
*/
InitialContext initCtx = new InitialContext(); //검색객체

DataSource ds = (DataSource) initCtx.lookup("java:comp/env/jdbc/myoracle"); //정해진 프로토콜로 외워야함
//새로운 접속이 아니라, 이미 풀에 존재하는 접속 객체를 대여하는것
